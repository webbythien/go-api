...
var (
	WorkerConfig *configs.WorkerConfig
)

func StartServer(cnf *configs.Worker) (*machinery.Server, error) {
...
func Execute(cnf *configs.Worker, tag string, concurrency int) error {
...
func Delay(queue, taskName string, fn interface{}, args ...interface{}) error {
	...
	worker := configs.NewWorker(queue, WorkerConfig)
